...

{
    ...
    for firewall in "${firewalls[@]}"; do
        ...
        # Memastikan pengecekan systemctl berada dalam satu baris perintah
        if command -v "$cmd" &> /dev/null && \
           systemctl is-enabled --quiet "$firewall" 2>/dev/null && \
           systemctl is-active --quiet "$firewall" 2>/dev/null; then
