...

{
    ...
    # Cek file utama sshd_config
    [ -e "/etc/ssh/sshd_config" ] && l_file="/etc/ssh/sshd_config" && f_sshd_files_chk

    # Cek file .conf di dalam direktori sshd_config.d
    while IFS= read -r -d $'\0' l_file; do
        [ -e "$l_file" ] && f_sshd_files_chk
    done < <(find /etc/ssh/sshd_config.d -type f -name '*.conf' \( -perm /0177 -o ! -user root -o ! -group root \) -print0 2>/dev/null)
