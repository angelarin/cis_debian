...

{
    ...
    # Di Debian, systemd-analyze biasanya mencari file tanpa prefix 'systemd/'
    l_systemd_config_file="journald.conf"
    a_parameters=("ForwardToSyslog=no")

    f_config_file_parameter_chk() {
        l_used_parameter_setting=""
        # Mencari konfigurasi efektif termasuk file di /etc/systemd/journald.conf.d/
        ...
        if [ -n "$l_used_parameter_setting" ]; then
            ...
            if grep -Pq -- "$l_parameter_value" <<< "$l_file_parameter_value"; then
                a_output+=(" - Parameter: \"${l_parameter_name}\" correctly set to: \"${l_file_parameter_value// /}\" in: \"$l_file_name\"")
            else
                a_output2+=(" - Parameter: \"${l_parameter_name}\" incorrectly set to: \"${l_file_parameter_value// /}\" in: \"$l_file_name\" (Should be: $l_parameter_value)")
            fi
        else
            a_output2+=(" - Parameter: \"$l_parameter_name\" is not explicitly set in journald configuration")
        fi
    }
