...

{
  ...
  # --- 1. Cek Keberadaan Konfigurasi auditd ---
  if [ -e "/etc/audit/auditd.conf" ]; then
    # Mengambil direktori log dari parameter log_file di auditd.conf
    l_audit_log_directory="$(dirname "$(awk -F= '/^\s*log_file\s*/{print $2}' /etc/audit/auditd.conf | xargs)")"

    # --- 2. Validasi Direktori Log ---
    if [ -d "$l_audit_log_directory" ]; then
      # Mencari file yang TIDAK dimiliki oleh user root
