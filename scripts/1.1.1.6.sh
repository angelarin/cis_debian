...
l_mod_type="fs"
# Menggunakan find yang lebih andal untuk menemukan path modul kernel
...
f_module_chk()
{
    # $1 adalah nama modul yang diperiksa (sekarang sudah disesuaikan)
    local l_chk_name="$1"
    a_showconfig=()
    while IFS= read -r l_showconfig; do
        a_showconfig+=("$l_showconfig")
    done < <(modprobe --showconfig | grep -P -- '\b(install|blacklist)\h+'"${l_chk_name//-/_}"'\b' 2>/dev/null)
    ...
    # Gunakan l_chk_name di sini
    if grep -Pq -- '\binstall\h+'"${l_chk_name//-/_}"'\h+(\/usr)?\/bin\/(true|false)\b' <<< "${a_showconfig[*]}"; then
    ...
    # Gunakan l_chk_name di sini
    if grep -Pq -- '\bblacklist\h+'"${l_chk_name//-/_}"'\b' <<< "${a_showconfig[*]}"; then
...
# 1. Cek keberadaan file modul di sistem (Menentukan apakah modul ada di disk)
...
if [ "$l_module_exists" = "y" ]; then
    # Mengatur nama modul yang akan dicari konfigurasinya: "overlayfs" -> "overlay"
    l_mod_chk_name="$l_mod_name" 
    [[ "$l_mod_name" =~ overlay ]] && l_mod_chk_name="${l_mod_name::-2}"

    # Perbaikan: Memanggil fungsi dengan nama modul kernel yang benar ("overlay")
