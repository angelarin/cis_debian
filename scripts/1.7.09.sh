...
check_setting() {
# $1: setting name (e.g., autorun-never)
# $2: dconf path (e.g., org/gnome/desktop/media-handling)
# $3: setting name for output
grep -Psrilq "^\h*$1\h*=\h*true\b" /etc/dconf/db/local.d/locks/* 2> /dev/null && \
echo "- \"$3\" is locked and set to true" || echo "- \"$3\" is not locked or not set to true"
}
