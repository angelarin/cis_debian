...
if [ ! -d "$TARGET_FILE" ]; then
    ...
    if [ "$L_UID" -eq "$EXPECTED_UID" ] && [ "$L_GID" -eq "$EXPECTED_GID" ]; then
        a_output+=(" - Uid ($L_UID) and Gid ($L_GID) are correctly set to root.")
    else
        a_output2+=(" - Uid ($L_UID) and/or Gid ($L_GID) are not set to root:root.")
        RESULT="FAIL"
    fi

    # 2. Cek izin (700 atau lebih ketat)
    if [ "$L_ACCESS_OCTAL" -le "$EXPECTED_MODE" ]; then
        a_output+=(" - Access ($L_ACCESS_OCTAL) is set to $EXPECTED_MODE or more restrictive (no group/other permissions).")
    else
