...
l_ssh_group_name="$(awk -F: '($1 ~ /^(ssh_keys|_?ssh)$/) {print $1}' /etc/group | head -n 1)" # Attempt to find SSH group
EXPECTED_OWNER="root"
...
# Tentukan mask izin berdasarkan grup
if [ "$l_file_group" = "$l_ssh_group_name" ]; then
    # Group khusus SSH: Mode 640 atau lebih ketat (Mask 0137)
    l_pmask="0137"
    l_maxperm="0640"
elif [ "$l_file_group" = "root" ]; then
    # Group root: Mode 600 atau lebih ketat (Mask 0177)
    l_pmask="0177"
    l_maxperm="0600"
else
    # Grup yang tidak diharapkan, asumsikan harus 600
    l_pmask="0177"
    l_maxperm="0600"
fi
...
if ssh-keygen -lf &>/dev/null "$l_file"; then
    file "$l_file" | grep -Piq -- '\bopenssh\h+([^#\n\r]+\h+)?private\h+key\b' && f_file_chk "$l_file"
fi
...
if [ "${#a_output2[@]}" -le 0 ]; then
    RESULT="PASS"
    [ "${#a_output[@]}" -le 0 ] && a_output+=(" - No openSSH private keys found in /etc/ssh.")
    NOTES+="PASS: ${a_output[*]}"
else
    RESULT="FAIL"
