...
TARGET_PATHS=("/etc/apparmor/" "/etc/apparmor.d/")
EXPECTED_TOTAL=2
FOUND_COUNT_DISK=0
...
f_check_files_modification() {
    ...
    for target in "${TARGET_PATHS[@]}"; do
        ...
        if [ "$source" = "disk" ]; then
            cmd="awk '/^ *-w/ && /${escaped_target}/ && / +-p *wa/ && / key=MAC-policy/{print \$0}' /etc/audit/rules.d/*.rules"
        else
            cmd="auditctl -l | awk '/^ *-w/ && /${escaped_target}/ && / +-p *wa/ && / key=MAC-policy/{print \$0}'"
        fi
        
        L_OUTPUT=$(eval "$cmd" 2>/dev/null)
        
        if [ -n "$L_OUTPUT" ] && echo "$L_OUTPUT" | grep -q "key=MAC-policy"; then
            a_output+=(" - $type: Rule for $target found.")
            found_count=$((found_count + 1))
        else
            a_output2+=(" - $type: Rule for $target (key=MAC-policy) is MISSING.")
        fi
    done
...
if [ "$FOUND_COUNT_DISK" -eq "$EXPECTED_TOTAL" ] && [ "$FOUND_COUNT_LOADED" -eq "$EXPECTED_TOTAL" ]; then
    ...
    RESULT="FAIL"
    NOTES+="FAIL: MAC configuration auditing failed (Disk: $FOUND_COUNT_DISK/$EXPECTED_TOTAL, Loaded: $FOUND_COUNT_LOADED/$EXPECTED_TOTAL). ${a_output2[*]}"
