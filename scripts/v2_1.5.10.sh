...
f_sysctl_chk()
{
    ...
    # Sesuai CIS, nilai harus 1, 2, atau 3
    ...
    for l_file in "${a_files[@]}"; do
        ...
        if [ -n "$l_option_value" ]; then
            if [[ "$l_option_value" =~ ^[1-3]$ ]]; then
                ...
                a_output2+=(" - Config file: \"$l_parameter_name = $l_option_value\" in \"$l_file\" (expected 1, 2, or 3)")
...
if [ "${#a_output2[@]}" -le 0 ]; then
    ...
    RESULT="FAIL"
    NOTES+="FAIL: Sysctl ptrace_scope configuration issues found | Reason(s): ${a_output2[*]}"
