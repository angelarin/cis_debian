...
f_sysctl_chk()
{
    ...
    # Menggunakan tac agar file dengan prioritas beban tertinggi (yang dibaca terakhir) diperiksa lebih dulu
    ...
    # Periksa setiap file untuk menemukan nilai parameter
    for l_file in "${a_files[@]}"; do
        ...
        if [ -n "$l_option_value" ]; then
            if [ "$l_option_value" = "0" ]; then
                ...
                # Berhenti setelah menemukan konfigurasi dengan prioritas tertinggi
                break
            else
...
if [ "${#a_output2[@]}" -le 0 ]; then
    ...
    RESULT="FAIL"
    NOTES+="FAIL: Sysctl suid_dumpable configuration issues found | Reason(s): ${a_output2[*]}"
