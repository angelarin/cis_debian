...
f_sysctl_chk()
{
    ...
    # Sesuai CIS, nilai harus 1 atau 2
    l_running_val=$(sysctl -n "$l_parameter_name" 2>/dev/null)
    if [[ "$l_running_val" =~ ^[1-2]$ ]]; then
        a_output+=(" - Running config: \"$l_parameter_name\" is set to \"$l_running_val\"")
    else
        a_output2+=(" - Running config: \"$l_parameter_name\" is set to \"${l_running_val:-NOT SET}\" (expected 1 or 2)")
    fi
    ...
    # Cek file UFW jika ada (khusus Debian sering menggunakan UFW untuk sysctl)
...
if [ "${#a_output2[@]}" -le 0 ]; then
    ...
    RESULT="FAIL"
    NOTES+="FAIL: Sysctl kptr_restrict configuration issues found | Reason(s): ${a_output2[*]}"
