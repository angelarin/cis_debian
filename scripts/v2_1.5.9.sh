...
f_sysctl_chk()
{
    ...
    # Sesuai CIS, nilai harus setara dengan 2 (Full Randomization)
    l_running_val=$(sysctl -n "$l_parameter_name" 2>/dev/null)
    if [ "$l_running_val" = "2" ]; then
        a_output+=(" - Running config: \"$l_parameter_name\" is set to \"2\"")
    else
        a_output2+=(" - Running config: \"$l_parameter_name\" is set to \"${l_running_val:-NOT SET}\" (expected 2)")
    fi
    ...
    # Cek file UFW jika ada (sering digunakan di Debian/Ubuntu)
    ...
    # Cari semua file .conf yang diproses oleh systemd-sysctl
    # Menggunakan tac agar file dengan prioritas beban tertinggi diperiksa lebih dulu
...
if [ "${#a_output2[@]}" -le 0 ]; then
    ...
    RESULT="FAIL"
    NOTES+="FAIL: Sysctl ASLR (randomize_va_space) configuration issues found | Reason(s): ${a_output2[*]}"
