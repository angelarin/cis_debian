...
f_sysctl_chk()
{
    ...
    if [ "$l_found_correct_file" = "n" ]; then
        a_output2+=(" - Config file: \"$l_parameter_name\" is not explicitly set in к any sysctl configuration file")
    fi
}
...
if [ "${#a_output2[@]}" -le 0 ]; then
    ...
    RESULT="FAIL"
    NOTES+="FAIL: IPv4 ICMP redirect sending is enabled or misconfigured | Reason(s): ${a_output2[*]}"
