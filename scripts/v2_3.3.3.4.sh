...
CHECK_ID="3.3.2.4"
DESCRIPTION="Ensure net.ipv6.conf.default.accept_redirects is configured"
# -----------------------------------------------------
...
f_sysctl_chk()
{
    # 1. Cek apakah IPv6 aktif di sistem
    if [ ! -d /proc/sys/net/ipv6 ]; then
        a_output+=(" - IPv6 is disabled on the system")
        return 0
    fi

    # 2. Audit Running Configuration
    ...
    # 3. Audit Configuration Files (Persistence)
...
# Jika IPv6 tidak aktif, maka dianggap PASS (Not Applicable)
if [[ "${a_output[*]}" == *"IPv6 is disabled"* ]]; then
    RESULT="PASS"
    NOTES="PASS: IPv6 is disabled, recommendation is not applicable"
else
    if [ "${#a_output2[@]}" -le 0 ]; then
        ...
        RESULT="FAIL"
        NOTES+="FAIL: IPv6 default ICMP redirect acceptance is enabled or misconfigured | Reason(s): ${a_output2[*]}"
