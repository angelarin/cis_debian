...
CHECK_ID="3.3.2.5"
DESCRIPTION="Ensure net.ipv6.conf.all.accept_source_route is configured"
# -----------------------------------------------------
...
if [[ "${a_output[*]}" == *"IPv6 is disabled"* ]]; then
    ...
    if [ "${#a_output2[@]}" -le 0 ]; then
        ...
        RESULT="FAIL"
        NOTES+="FAIL: IPv6 accept source route (all) is enabled or misconfigured | Reason(s): ${a_output2[*]}"
